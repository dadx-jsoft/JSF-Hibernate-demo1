...
public class DatabaseOperations {
	...
	public void addStudentInDb(Student studentObj) {
		try {
			...
			System.out
					.println("Student Record With Id: " + studentObj.getId() + " Is Successfully Created In Database");
	...
	public List<Student> getStudentById(int studentId) {
		...
		List particularStudentList = new ArrayList<Student>();
		try {
			transObj = sessionObj.beginTransaction();
			Query queryObj = sessionObj.createQuery("from Student where id= :student_id").setInteger("student_id",
					studentId);
	...
	public void updateStudentRecord(Student updateStudentObj) {
		try {
			...
			// XHTML Response Text
			FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("updatedStudentRecord",
					"Success");
		} catch (Exception exceptionObj) {
			exceptionObj.printStackTrace();
		} finally {
